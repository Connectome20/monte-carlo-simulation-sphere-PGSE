...
d_mean = 10;            % mean diameter, um
f      = 0.4;           % volume fraction

% Membrane permeability, um/ms
kappa = [0.02, 0.05, 0.10];
...
Din = [0.5 1 1.5 2];    % ICS diffusivity, um^2/ms
Dex = [0.5 1 1.5 2];    % ECS diffusivity, um^2/ms
...
for i = 1:numel(Din)
    ...
    for j = 1:numel(Dex)
        ...
        for k = 1:numel(kappa)
            ...
            maxdensity = fi;    % maximal intra-spherical volume fraction
            ...
            fprintf(fileID,sprintf('%g\n%u\n%u\n%u\n%g\n%g\n%g\n%u\n%u\n',dt,TN,NPar,Nbvec,Dini,Dexi,kappai,pinit,threadpb));
            fclose(fileID);
        end
    end
end
...
for i = 1:numel(Din)*numel(Dex)*numel(kappa)
